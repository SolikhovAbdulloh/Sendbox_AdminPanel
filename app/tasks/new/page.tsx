...
const analysisPackages = [
  { value: "exe", label: "Windows Executable (exe)", description: "Analyzes Windows executable files" },
  { value: "dll", label: "Windows DLL (dll)", description: "Analyzes Windows DLL files" },
  { value: "pdf", label: "PDF Document (pdf)", description: "Analyzes PDF documents for malicious content" },
  { value: "doc", label: "Office Document (doc)", description: "Analyzes Microsoft Office documents" },
  { value: "zip", label: "Archive (zip)", description: "Extracts and analyzes archive contents" },
  { value: "js", label: "JavaScript (js)", description: "Analyzes JavaScript files" },
  { value: "url", label: "URL Analysis (url)", description: "Analyzes URLs and captures network traffic" },
]
...
export default function NewTaskPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // For demo purposes, we'll just redirect back to the active tasks page
    console.log({
      type: activeTab,
      file,
      url,
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-6">
            <Tabs defaultValue="file" value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="url" className="space-y-6 pt-4">
                <div className="grid gap-3">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Enter a URL to analyze. The system will visit the URL and capture network traffic and behavior.
                  </p>
                </div>
              </TabsContent>
            </Tabs>
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-sm">
                        <p>Select the appropriate analysis package based on the file type you're analyzing.</p>
            ...
            <div className="grid gap-3">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Comma-separated list of options to pass to the analyzer (e.g., timeout=120,enforce_timeout=1)
              </p>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="grid gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {preScript && (
                    <Button type="button" variant="ghost" size="icon" onClick={() => setPreScript(null)}>
                      <X className="h-4 w-4" />
                    </Button>
                  )}
                </div>
                <p className="text-xs text-muted-foreground">This script will run before the main analysis starts</p>
              </div>
              ...
              <div className="grid gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {duringScript && (
                    <Button type="button" variant="ghost" size="icon" onClick={() => setDuringScript(null)}>
                      <X className="h-4 w-4" />
                    </Button>
                  )}
                </div>
                <p className="text-xs text-muted-foreground">This script will run during the analysis process</p>
              </div>
            </div>
          </CardContent>
